...
export function MailingListForm({ heading, subheading }: Props) {
  ...
  return (
    <div className="max-w-lg mx-auto text-center">
      <h2 className="font-display text-3xl lg:text-4xl italic text-ivory mb-3">{heading}</h2>
      {subheading && (
        <p className="font-sans text-sm text-ivory/60 mb-8 leading-relaxed">{subheading}</p>
      )}
        ...
        <form action={action} className="flex flex-col sm:flex-row gap-3 justify-center">
          <div className="flex-1 max-w-xs">
            ...
            <input
              ...
              required
              className="w-full bg-ivory/10 border border-ivory/20 text-ivory placeholder:text-ivory/30 px-4 py-3 text-sm font-sans focus:outline-none focus:border-ivory/60 transition-colors"
            />
          </div>
          <button
            type="submit"
